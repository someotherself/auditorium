...
use maudio::{
    ...
    data_source::{
        ...
        sources::waveform::{WaveForm, WaveFormOps},
    },
...
impl LiveHost for WaveInner {
    fn is_shutdown(&self) -> bool {
        self.is_shutdown.load(Ordering::Relaxed)
    }
}

impl HostDispatcher for WaveInner {
...
impl Wave {
    ...
    pub fn set_volume(&self, volume: f32) -> HostResult<()> {
        ...
        self.0.call_store_impl(device_id, move |store| {
            match store.mut_nodes().values.get_mut(&id).unwrap() {
                HostedNodes::AttachedWave(value) => value
                    .as_node()
    ...
    pub fn get_volume(&self) -> HostResult<f32> {
        ...
        self.0.call_store_impl(device_id, move |store| {
            match store.mut_nodes().values.get_mut(&id).unwrap() {
                HostedNodes::AttachedWave(value) => Ok(value.as_node().output_bus_volume(0)),
                _ => unreachable!(),
            }
        })
    }
    ...
    pub fn start_audio(&self) -> HostResult<()> {
        ...
        self.0.call_store_impl(device_id, move |store| {
            match store.mut_nodes().values.get_mut(&id).unwrap() {
                HostedNodes::AttachedWave(value) => {
    ...
    pub fn set_type(&self, wave_type: WaveFormType) -> HostResult<()> {
        let id = self.0.id;
        let device_id = self.0.store_id;
        self.0.call_store_impl(device_id, move |store| {
            match store.mut_nodes().values.get_mut(&id).unwrap() {
...
impl PcmSource<f32> for TrackedSource<WaveForm<f32>> {
    fn fill_pcm_frames(
        &mut self,
