//! Pulsewave generator
//!
//! This module provide [`Pulse`], a handle for controlling a Pulseform
//! generator managed by a [`Host`](crate::host).
//!
//! `Pulse` does not directly own the underlying pulse generator. Instead, the
//! source is stored in the node graph associated with the device that created
//! it. Operations performed through `Pulse` are dispatched to the host thread,
//! where the source and its node can be safely accessed.
//!
//! `Pulse` can be cloned. Each clone refers to the same underlying pulse generator.
//! The source remains available while at least one `Pulse` handle exists.
//!
...
///
/// `Pulse` provides control over an pulse generator stored in a device's node
...
///
/// `Pulse` does not directly own the underlying source. Instead, operations are
/// dispatched to the host thread, which owns the device and its node graph.
///
/// `Pulse` can be cloned, and all clones refer to the same underlying source.
