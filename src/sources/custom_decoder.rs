...
use symphonia::{
    core::{
        ...
        errors::Error::{self as SymphoniaError},
...
impl<'stream> SymphoniaDecoder<'stream> {
    fn new(stream: DecoderStream<'stream>) -> MaResult<SymphoniaDecoder<'stream>> {
        ...
        let (track_id, codec_params, channels, sample_rate, num_frames) = {
            let track = format
        ...
        Ok(SymphoniaDecoder {
            ...
            channels,
            sample_rate,
            len_frames: num_frames.unwrap(), // we are guaranteed to have a lenght here
...
impl<'stream> PcmSource<f32> for SymphoniaDecoder<'stream> {
    fn fill_pcm_frames(&mut self, out: &mut [f32], ctx: &mut SourceContext) -> MaResult<usize> {
        if !out.len().is_multiple_of(self.channels) {
